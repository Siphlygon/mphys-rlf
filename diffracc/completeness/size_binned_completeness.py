...
class SizeBinnedCompleteness(CompletenessEstimator):
    ...
    def __init__(self,
        ...
        override_data : bool, optional
            Whether to override the data in ImageDataArrays with data from the provided paths. Defaults to False.
        paths_to_use : list[Path] | None, optional
            A list of paths to use if override_data is True. Should contain three paths:
            [ang_size_path, model_image_path, model_flux_path]. Defaults to None.
        ...
        if override_data:
            ...
            # Get sizes
            self.logger.info("Getting angular sizes...")
            ...
            self.logger.info(f"Number of sources with matching indices across all datasets: {len(common_indices)}")
    ...
    def estimate_size_binned_completeness(self,
        ...
        show_progress : bool, optional
            Whether to show a progress bar during the completeness estimation. Defaults to True.

        Returns
        -------
        list[tuple]
            A list of tuples containing the log bin centers, completeness values, y errors, and fitted parameters for
            each size bin.
        """
        # Store results
        ...
        for i in range(len(self.size_bins_edges) - 1):
            ...
            self.logger.info(f"Model fluxes in this size bin: {fluxes_in_bin}")
            self.logger.info(f"Max model flux in this size bin: {np.max(fluxes_in_bin)} mJy/beam")

            # Dynamically set the model images and fluxes in self.data to be the sources in this size bin, so that we
            # can reuse the inherited completeness estimation code
    ...
    def plot_size_binned_completeness(self, completeness_results: list[tuple]):
        ...
        Parameters
        ----------
        completeness_results : list[tuple]
            A list of tuples containing the log bin centers, completeness values, y errors, and fitted parameters for
            each size bin.
        """
        # Build a 256-colour palette (HSV) with relatively low brightness so colours are darker, then sample
        # `self.max_bins` well-separated colours from that palette regardless of how many bins are requested.
        n_palette = 256
        ...
        plt.xscale('log')
        plt.ylim(-0.01, 1.05)  # Set y-axis limits to slightly beyond [0, 1] for better visibility of error bars
        plt.xlim(10**-1, 10**2)  # Set x-axis limits to match the range of flux densities
