...
class FitsViewer:
    ...
    *files: list[Path]
    ...
    SORT_BY_PEAK_FLUX_UNSCALED = 2

    def _sorted_files(self, sorting: int) -> list[Path]:
        """
        Order self.files according to the requested sorting mode.

        Parameters
        ----------
        sorting : int
            One of NO_SORTING, SORT_BY_FLUX_SCALED, SORT_BY_PEAK_FLUX_UNSCALED.

        Returns
        -------
        list[Path]
            self.files reordered according to sorting.

        Raises
        ------
        ValueError
            If sorting is not one of the recognised modes.
        """
        if sorting == FitsViewer.NO_SORTING:
            return list(self.files)
        elif sorting == FitsViewer.SORT_BY_FLUX_SCALED:
            return sorted(self.files, key=lambda file : rfa.get_fits_primaryhdu_header(file, 'FXSCLD'))
        elif sorting == FitsViewer.SORT_BY_PEAK_FLUX_UNSCALED:
            return sorted(self.files, key=lambda file : np.max(rfa.get_fits_primaryhdu_data(file)))
        else:
            raise ValueError(
                f"Unknown sorting mode {sorting!r}; expected one of NO_SORTING ({FitsViewer.NO_SORTING}), "
                f"SORT_BY_FLUX_SCALED ({FitsViewer.SORT_BY_FLUX_SCALED}), or SORT_BY_PEAK_FLUX_UNSCALED "
                f"({FitsViewer.SORT_BY_PEAK_FLUX_UNSCALED})")
    ...
    def show_image_grid(self,
        ...
        sorting : int = 0
            NO_SORTING (0) leaves files in their given order, SORT_BY_FLUX_SCALED (1) sorts by each file's FXSCLD
            header, and SORT_BY_PEAK_FLUX_UNSCALED (2) sorts by each file's raw peak pixel value. Raises ValueError
            for any other value.
        ...
        # Reuse the cached files/data unless the requested sorting differs from what's cached
        if self.__data_cache is not None and self.__sorting == sorting:
            files = self.__files_cache
            data = self.__data_cache
        else:
            files = self._sorted_files(sorting)
            # Order is important here, use index iter just in case
            data = [rfa.get_fits_primaryhdu_data(files[i]) for i in range(len(files))]
            self.__files_cache = files
            self.__data_cache = data
            self.__sorting = sorting

        titles = [None if no_titles else files[i].name for i in range(len(files))] # Order is also important here
...
if __name__ == "__main__":
    ...
    parser.add_argument("--rows",
                        help="How many rows to display the fits images in, default -1 automatically makes the image " \
                        + " square-like", type=int, default=-1)
    parser.add_argument("--resolution", help="Vertical pixel resolution to display the image grid at, default 1080",
    ...
    parser.add_argument("-o", "--outfile",
                        help="Where to write the output file to. By default does not write figure to a file.",
    ...
    parser.add_argument("--upper-bound", help="Specify upper bound on images, by default no bound",
                        type=float, default=None)
    parser.add_argument("--no-titles",
                        help="Use this flag to not display titles on the images, e.g. for cluttered grids",
                        action='store_true', default=False)
    parser.add_argument("--no-ticks",
                        help="Use this flag to not display axis ticks on the images, e.g. for cluttered grids",
                        action='store_true', default=False)
    parser.add_argument("--sorting",
                        help="0 does no sorting, 1 sorts by peak flux scaled, 2 sorts by unscaled peak flux. Note " + \
                            " that these should be identical up to a minus sign.", type=int, default=0)
    parser.add_argument("FILES",
                        help="Any number of files, or directories to search through, to be read and displayed in a "+ \
                            " grid", nargs='*')
