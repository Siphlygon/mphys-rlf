"""
This file was created by Ashley and Luna and defines functions neccesary to convert the LOFAR h5 dataset that comes with
the project into FITS files needed for PyBDSF, as well as a more specific function which implements the first function
and checks for compliance with the arguments, and a main function which uses the config file
"""
import argparse
import configparser
import math
...
def convert_lofar_h5_to_fits( lofar_data_h5: Path,
    ...
    fits_output_dir, and groups the images into directories by descending sizes. Saves the converted images to the
    subdirectory of the fits_output_dir specified by subdir.
    ...
    with h5py.File( str( lofar_data_h5 ), 'r' ) as h5:
        images : np.ndarray = h5[ 'images' ]  # type: ignore
...
def validate_lofar_fits_images( subdir: str,
                               cutoff: int | None,
                               bin_size: int | None ):
    """
    ...
    for bin in fits_dataset_folder.glob( "*" ):
        ...
        print( f"Bin {lower_bound}-{upper_bound} ({bin.parent/bin.name}) not compliant with bin size {bin_size},"
                " removing directory..." )
        shutil.rmtree( fits_dataset_folder )
        break
...
if __name__ == "__main__":
    ...
    bin_size = int( config[ 'FOLDER_SIZE' ] )
    vm_dataset_subdir = config[ 'DATASET_SUBDIR' ]
